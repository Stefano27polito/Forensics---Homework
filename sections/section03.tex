...
This laboratory can reuse the CAINE14/Kali VM already configured in Laboratory 2. \\
...
\subsection{Base Forensic VM --- Distribution Choice and Write-Blocking (recap from Laboratory 2)}
...
\subsection{Setting Up a Local S3-Compatible Emulator (MinIO)}
...
\noindent
Then configure it with the MinIO root credentials. Run each line separately, exactly as in
...
\begin{questionbox}
Why must every command in this section explicitly specify \texttt{--endpoint-url}? \\
What would happen if a student forgot this flag, and why is this exact mistake also a realistic risk during a genuine cloud investigation involving multiple provider accounts or regions?
\end{questionbox}
